...
import {
  addSchedulePick,
  removeSchedulePick,
  updateSchedulePickNote,
} from '../../lib/data/schedulePicks';
...
function SchedulePickNoteEditor({ pick }: SchedulePickNoteEditorProps) {
  ...
  async function saveNote() {
    ...
    try {
      ...
      // Keep the draft so a failed request never erases the user's input.
      setSaveError('Could not save the note. Please try again.');
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="artist-detail__note-field">
      <label
        htmlFor={`pick-note-${pick.id}`}
        className="artist-detail__note-label"
      >
        Note (optional)
      </label>
      ...
      <input
        ...
        type="text"
        maxLength={300}
        placeholder="e.g. leaving at 7:45 for the train"
        value={draft}
        onChange={(event) => {
...
export function ArtistDetail() {
  ...
  const setActiveFriendId = useAppStore(
    (state) => state.setActiveFriendId,
  );
  const [newSongTitle, setNewSongTitle] = useState('');

  const pick = artist
    ? picks.find((candidate) => candidate.artistId === artist.id)
    : undefined;
  ...
  const stage = stages.find(
    (candidate) => candidate.id === artist.stageId,
  );
  const friendById = new Map(
    friends.map((friend) => [friend.id, friend]),
  );
  const pickedByFriend = pick
    ? friendById.get(pick.addedBy)
    : undefined;
  ...
  async function togglePick() {
    ...
    await addSchedulePick({
      artistId: artist.id,
      addedBy: activeFriendId,
    });
  }
    ...
    <Sheet onClose={close} className="artist-detail">
      {(requestClose) => (
        <>
          <button
            type="button"
            className="artist-detail__close"
          ...
          <header className="artist-detail__header">
            ...
            <div className="artist-detail__meta">
              {stage && (
                <span className="artist-detail__stage">
                  {stage.name}
                </span>
              )}
          ...
          <button
            type="button"
            className={
              `artist-detail__pick-toggle` +
              `${pick ? ' artist-detail__pick-toggle--active' : ''}`
            }
            onClick={togglePick}
          ...
          >
            <Star
              ...
              className="pop-in"
              size={18}
              fill={pick ? 'var(--lolla-coral)' : 'none'}
              strokeWidth={2}
            />
          ...
          <section className="artist-detail__section">
            ...
            {songs.length === 0 ? (
              <p className="artist-detail__empty">
                No songs added yet.
              </p>
            ) : (
              <ul className="artist-detail__song-list">
                {songs.map((song) => (
                  <li
                    key={song.id}
                    className="artist-detail__song"
                  >
                    <div className="artist-detail__song-main">
                      <span className="artist-detail__song-title">
                        {song.title}
                      </span>

                      <AttributionTag
                        friend={friendById.get(song.addedBy)}
                      />
                    </div>
            ...
            <form
              className="artist-detail__song-form"
            ...
            >
              ...
              <button
                type="submit"
                disabled={
                  !newSongTitle.trim() || !activeFriendId
                }
              >
                Add
              </button>
